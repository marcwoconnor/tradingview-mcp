...
export function registerChartTools(server) {
  server.tool('chart_get_state', 'Get current chart state (symbol, timeframe, chart type, indicators)', {}, wrap(core.getState));
  ...
  }, wrap(core.manageIndicator));

  server.tool('chart_get_visible_range', 'Get the visible date range (unix timestamps) and bars range on the chart', {}, wrap(core.getVisibleRange));
  ...
  }, wrap(core.scrollToDate));

  server.tool('symbol_info', 'Get detailed metadata about the current symbol (name, exchange, type, description)', {}, wrap(core.symbolInfo));
